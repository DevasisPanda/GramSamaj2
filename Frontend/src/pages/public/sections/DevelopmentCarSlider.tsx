  ...
  return (
    <div className="card-surface p-6">
      <h3 className="font-bold text-lg text-gradient-saffron flex items-center gap-2">
        <Car className="h-5 w-5" /> {DEVELOPMENT_CAR.title}
      </h3>
      <p className="text-sm text-ink/50">{DEVELOPMENT_CAR.subtitle}</p>

      <p className="mt-3 text-sm text-ink/70 leading-relaxed">{DEVELOPMENT_CAR.intro}</p>

      {/* Visual car + health */}
      <div className="mt-6 rounded-2xl bg-gradient-to-br from-saffron-50 to-forest-50 p-6">
        <div className="flex items-center justify-center gap-6 flex-wrap">
          {/* Driver mood */}
          <div className="text-center">
            <div className={cn(
              'grid h-16 w-16 place-items-center rounded-full text-3xl transition-all',
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl bg-forest-50 p-3 flex gap-2">
          ...
          <p className="text-xs text-ink/70">{DEVELOPMENT_CAR.blessing}</p>
        </div>
        <div className="rounded-xl bg-red-50 p-3 flex gap-2">
          <Skull className="h-4 w-4 text-red-600 shrink-0 mt-0.5" />
          <p className="text-xs text-ink/70">{DEVELOPMENT_CAR.curse}</p>
        </div>
      </div>

      <ul className="mt-4 space-y-1.5">
        {DEVELOPMENT_CAR.facts.map((f, i) => (
          <li key={i} className="text-xs text-ink/60 leading-relaxed flex gap-2">
            <span className="text-saffron-500 mt-0.5">&#x25C6;</span> {f}
          </li>
        ))}
      </ul>
    </div>
