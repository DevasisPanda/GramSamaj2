...
export function HumanitySection() {
  return (
    <div className="bg-white border border-forest-900 mb-1 p-1.5">
      ...
      <p className="text-[10px] text-gray-800 mb-1.5 leading-snug">
        ...
        {CARDS.map((card) => (
          <div key={card.id} className="border-b border-gray-200 pb-1">
            <div className="font-bold text-forest-900 flex items-center justify-between">
              <span>&bull; {card.title}</span>
            </div>
            <p className="text-gray-700 leading-tight mb-0.5">{card.summary}</p>
            <Accordion type="single" collapsible>
              <AccordionItem value={card.id} className="border-0">
                ...
                <AccordionContent>
                  <p className="text-[9px] text-gray-700 leading-tight border-t pt-0.5 mt-0.5">{card.body}</p>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
          </div>
      ...
      <div className="border-l-2 border-saffron-600 bg-saffron-50 p-1 text-[9px] italic font-semibold text-saffron-950 mb-1.5">
